...
import 'package:tunein/data/class/Team.dart';
import 'package:tunein/data/class/TuneInUser.dart';
import 'package:tunein/data/data.dart';
import 'package:tunein/page/MyTeamMemberVer.dart';
import 'package:tunein/page/TeamConfigPage.dart';
import 'package:tunein/page/TeamListPage.dart';
...
class MyProfilePage extends StatefulWidget {
  MyProfilePage({
    Key? key,
    required TuneInUser user,
  })  : _user = user,
        super(key: key);

  final TuneInUser _user;
        ...
        if (snapshot.data != null) {
          ...
          return ListView(
            ...
            children: <Widget>[
              ...
              /// 이름
              MyProfileInfo(title: '이름', content: widget._user.name!),

              /// 연락처
              MyProfileInfo(title: '연락처', content: widget._user.nickName!),//widget._user.position!

              /// 주소
              MyProfileInfo(title: '주소', content: widget._user.address!),

              /// 이메일
              MyProfileInfo(title: '이메일', content: widget._user.email!),

              /// 경력
              MyProfileInfo(title: '경력', content: widget._user.career! + '년'),

              /// 역할
              MyProfileInfo(title: '역할', content: widget._user.roll!),

              /// 분야
              MyProfileInfo(title: '분야', content: widget._user.position!),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: <Widget>[
          ...
          ElevatedButton(
              onPressed:() async {
                await FirebaseFirestore.instance.collection('Account').doc(uid).get().then((DocumentSnapshot value){
                  if(value.data()!.containsKey('Teams')){
