...
import 'package:tunein/widget/net_Container.dart';
...
class _UploadQuestionState extends State<UploadQuestionPage> {
  ...
  Future _getImage() async{
    ...
    Reference storageReference = _firebaseStorage.ref()
  ...
  Future _uqpPostUpload() async{
    ...
    final DocumentReference docUQP = FirebaseFirestore.instance.collection('meta').doc('uqp');
    String nickName = '';
    final String document = "${now.toString()}__$uid";
    await FirebaseFirestore.instance.collection('Account').doc(uid).get().then((DocumentSnapshot ds) {
      nickName = ds.data()!["nickName"];
    });
    docUQP.update({"count": FieldValue.increment(1)}).then((value){
      setSearchPageUQPCount().then((value){
        FirebaseFirestore.instance.collection('uqpPost').doc(document).set({
          'Writer_uid':uid,
          'Writer':nickName,
          ...
          'Time' : now,
          'TotalCount' : searchPageUQPCount + 1,
        });
      });
    });
  }
